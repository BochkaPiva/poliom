#!/usr/bin/env python3
"""
Упрощенная обработка необработанных документов с улучшенным алгоритмом чанкинга
Обходит проблему с PostgreSQL векторными типами
"""
...
def process_document_simple(document_id: int):
    """Упрощенная обработка документа без проверки старых чанков"""
    print(f"🔄 Упрощенная обработка документа ID: {document_id}")
    ...
    try:
        ...
        # Удаляем старые чанки через SQL (обходим проблему с SQLAlchemy)
        print("🗑️ Удаляем старые чанки через SQL...")
        try:
            db.execute(text("DELETE FROM document_chunks WHERE document_id = :doc_id"), {"doc_id": document_id})
            db.commit()
            print("✅ Старые чанки удалены")
        except Exception as e:
            print(f"⚠️ Предупреждение при удалении старых чанков: {e}")
...
def main():
    """Основная функция"""
    print("🚀 Упрощенная обработка документа ID 3 с улучшенным алгоритмом\n")
    
    # Обрабатываем документ с ID 3
    document_id = 3
    success = process_document_simple(document_id)
