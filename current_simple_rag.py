    ...
    def search_relevant_chunks(self, question: str, limit: int = 15) -> List[Dict]:
        ...
        try:
            ...
            for row in result:
                ...
                self.logger.info(f"Векторный поиск: найден чанк с similarity={row.similarity:.3f}")
                
                # Понижаем порог схожести с 0.65 до 0.4 для лучших результатов
                if (row.similarity > 0.4 and 
                    ...
                    self.logger.info(f"Чанк добавлен в результаты (similarity={row.similarity:.3f})")
                else:
                    self.logger.info(f"Чанк отфильтрован: similarity={row.similarity:.3f}, relevant={self._is_relevant_content(row.content, question)}")
...
"""
            ...
            self.logger.info(f"🔍 ОТЛАДКА: Начинаем формирование sources из {len(top_chunks)} чанков")
            ...
            for i, chunk in enumerate(top_chunks):
                ...
                if document:
                    self.logger.info(f"📄 Чанк {i+1}: document_id={document.id}, title='{document.title}', seen={document.title in seen_documents}")
                    
                    if document.title not in seen_documents:
                        sources.append({
                        ...
                        self.logger.info(f"✅ Добавлен источник: '{document.title}'")
                    else:
                        self.logger.info(f"⏭️ Пропущен дубликат: '{document.title}'")
                else:
                    self.logger.warning(f"❌ Документ не найден для chunk_id={chunk.get('document_id')}")
            
            self.logger.info(f"📊 ИТОГО: sources={len(sources)}, files={len(files)}")
            self.logger.info(f"📋 Sources titles: {[s['title'] for s in sources]}")
            self.logger.info(f"📋 Files titles: {[f['title'] for f in files]}")
